...
const nohinshoFilterByDateInput = document.getElementById('nohinsho-filter-by-date');
...
let authToken = window.authToken || localStorage.getItem(AUTH_TOKEN_KEY) || '';
...
function isNohinshoDateFilteringEnabled() {
    return !!nohinshoFilterByDateInput?.checked;
}
...
function showAdminToast(message, type = 'info', autoDismiss = 4000) {
...
function filterSortedRecords(records) {
    const selectedDate = isNohinshoDateFilteringEnabled() ? (nohinshoDateInput.value || todayDateString()) : '';
...
async function openScanModal() {
    ...
    if (btnScanCropSelect) btnScanCropSelect.classList.add('hidden');
...
function startLiveBarcodeScanning() {
    ...
    scanIntervalId = setInterval(() => {
        ...
        const dataUrl = captureVideoFrameAsDataUrl(0.8);
        if (!dataUrl) return;
        decodeBarcode(dataUrl, (code) => {
...
function captureVideoFrameAsDataUrl(quality = 1.0) {
...
function decodeBarcode(dataUrl, callback) {
...
function decodeBarcodeAsync(dataUrl) {
    return new Promise((resolve) => {
        decodeBarcode(dataUrl, (code) => resolve((code || '').trim()));
    });
}
...
function setScanCropMode(active) {
    ...
    if (scanCropOverlay) {
        scanCropOverlay.classList.toggle('active', scanCropActive);
    }
}
...
function beginScanCropSelection() {
    if (!scanCapturedDataUrl) {
        showAdminToast('先に撮影してください。', 'info', 2200);
        return;
    }
    setScanCropMode(true);
    clearScanCropRect();
    if (btnScanCropApply) btnScanCropApply.classList.remove('hidden');
    showAdminToast('画像上をドラッグして切り抜き範囲を選択してください。', 'info', 2600);
}
...
async function applyScanCropSelection() {
    if (!scanCropRect) {
        showAdminToast('切り抜き範囲を選択してください。', 'info', 2200);
        return;
    }
    const imgRect = convertOverlayRectToImageRect();
    if (!imgRect) {
        showAdminToast('画像上の有効な範囲を選択してください。', 'error', 2500);
        return;
    }
    ...
    const detected = (await decodeBarcodeAsync(croppedDataUrl)) || '';
    if (detected) {
        scanDetectedCode = detected;
        scanBarcodeCodeEl.textContent = detected;
        scanBarcodeBadge.classList.remove('hidden');
        if (!scanFilenameInput.value.trim()) scanFilenameInput.value = detected;
        showAdminToast('切り抜きを適用しました。バーコードを更新しました。', 'success', 2600);
        return;
    }

    showAdminToast('切り抜きを適用しました。', 'success', 2200);
}

async function handleScanCapture() {
    const dataUrl = captureVideoFrameAsDataUrl(1.0);
    if (!dataUrl) {
        showAdminToast('カメラからフレームを取得できませんでした。', 'error');
        return;
    }
    ...
    if (btnScanCropSelect) btnScanCropSelect.classList.remove('hidden');
    if (btnScanCropApply) btnScanCropApply.classList.add('hidden');
    btnScanRetake.classList.remove('hidden');
    btnScanUpload.classList.remove('hidden');
    scanBarcodeBadge.classList.add('hidden');

    showAdminToast('バーコードを解析中...', 'info', 2500);
    decodeBarcode(dataUrl, (code) => {
        ...
        if (detected) {
            ...
            showAdminToast(`✅ バーコード: ${detected}`, 'success', 3000);
        } else {
            showAdminToast('バーコードは検出できませんでした。手動でファイル名を入力してください。', 'info', 4000);
        }
    });
}
...
async function handleScanUpload() {
    ...
    if (!rawName) {
        const detected = (await decodeBarcodeAsync(capturedDataUrl)) || (scanDetectedCode || '').trim();
        if (detected) {
            rawName = detected;
            scanFilenameInput.value = detected;
        }
    }
...
async function boot() {
    ...
    try {
        ...
        console.warn('Admin records load failed:', error);
    }
    ...
    if (nohinshoFilterByDateInput) nohinshoFilterByDateInput.checked = false;
    renderUsers();
    renderNohinshoRecords();
    setActiveTab('users');
...
if (nohinshoFilterByDateInput) {
    nohinshoFilterByDateInput.addEventListener('change', () => {
        if (nohinshoFilterByDateInput.checked && !nohinshoDateInput.value) {
            nohinshoDateInput.value = todayDateString();
        }
        selectedRecordId = null;
        clearNohinshoDetailForm();
...
// Scan modal events
btnScanCapture.addEventListener('click', handleScanCapture);
...
btnScanRetake.addEventListener('click', handleScanRetake);
btnScanUpload.addEventListener('click', handleScanUpload);
